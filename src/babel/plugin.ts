...
import {
    ...
    taggedTemplateVisitor,
} from "./plugin/visitors/tw.js";
import {
...
function collectTailwindConfigSearchRoots(
    filename: string,
    babelConfigDir: string,
    cwd: unknown,
    root: unknown,
): string[] {
    const candidates = [babelConfigDir, cwd, root]
        .filter(
            (value): value is string =>
                typeof value === "string" && value.length > 0,
        )
        .map((dir) => (isAbsolute(dir) ? dir : resolve(dir)));

    if (filename.length > 0) {
        candidates.push(dirname(filename));
    }
...
export default function reactNativeTailwindBabelPlugin(
    ...
    babelConfigDir = process.cwd(),
): PluginObj<PluginState> {
    // Color scheme hook configuration from plugin options
    const colorSchemeImportSource =
        options?.colorScheme?.importFrom ?? "react-native";
    const colorSchemeHookName =
        options?.colorScheme?.importName ?? "useColorScheme";
    ...
    const configProviderImportName =
        ...
        visitor: {
            Program: {
                enter(path, state) {
                    ...
                    const tailwindConfigSearchRoots =
                        collectTailwindConfigSearchRoots(
                            filename,
                            babelConfigDir,
                            state.file.opts.cwd,
                    ...
                    if (state.configProviderEnabled) {
                        ...
                        if (tailwindConfigPath) {
                            const genPath =
                                getConfigModulePath(tailwindConfigPath);
                    ...
                    for (const stmt of path.node.body) {
                        ...
                        if (
                            stmt.source.value === state.colorSchemeImportSource
                        ) {
                        ...
                        if (
                            ...
                            for (const spec of stmt.specifiers) {
                                if (
                                    ...
                                    if (
                                        importedName === "tw" ||
                                        importedName === "twStyle"
                                    ) {
                                        state.twImportNames.add(
                                            spec.local.name,
                                        );
                                    }
                                }
