 ...
 */
export function warnUnsupportedThemeKeys(
    config: TailwindConfig,
    configPath: string,
): void {
    if (
        process.env.NODE_ENV === "production" ||
        warnedConfigPaths.has(configPath)
    ) {
        return;
    }
...
export function findTailwindConfig(
    startDir: string,
    additionalStartDirs: string[] = [],
): string | null {
    const startDirs = [startDir, ...additionalStartDirs]
        .filter(
...
export function loadTailwindConfig(configPath: string): TailwindConfig | null {
    ...
    try {
        ...
        const resolved: TailwindConfig =
            "default" in config ? config.default : config;
        ...
        if (process.env.NODE_ENV !== "production") {
            console.warn(
                `[react-native-tailwind] Failed to load config from ${configPath}:`,
                error,
            );
        }
 ...
 */
export function extractCustomTheme(
    filename: string,
    additionalStartDirs: string[] = [],
): CustomTheme {
    const projectDir = path.dirname(filename);
    const configPath = findTailwindConfig(projectDir, additionalStartDirs);
    ...
    const fontFamily =
        config.theme.extend?.fontFamily ?? config.theme.fontFamily ?? {};
    ...
    const fontSize =
        config.theme.extend?.fontSize ?? config.theme.fontSize ?? {};
