...
export type PluginState = PluginPass & {
    ...
    functionComponentsNeedingWindowDimensions: Set<
        NodePath<BabelTypes.Function>
    >;
 ...
 * @param tailwindConfigSearchRoots - Additional directories to search when resolving tailwind.config.*
...
export function createInitialState(
    ...
    tailwindConfigSearchRoots: string[] = [],
): Partial<PluginState> {
    ...
    const stylesIdentifier =
        options?.stylesIdentifier ?? DEFAULT_STYLES_IDENTIFIER;

    // Load custom theme from tailwind.config.*
    const customTheme = extractCustomTheme(filename, tailwindConfigSearchRoots);
        ...
        for (const rule of options.componentClassToPropMapping) {
            ...
            if (hasInvalidPattern) {
                ...
                const validMapping = Object.fromEntries(
                    Object.entries(rule.mapping).filter(([, pattern]) =>
                        pattern.includes("*"),
                    ),
                );
