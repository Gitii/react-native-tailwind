...
function transform(code: string, options?: Parameters<typeof baseTransform>[1], includeJsx = false): string {
...
function transformWithConfig(
  ...
  return baseTransformWithConfig(code, configProviderImportFrom, options, TEST_FILE.filename);
}
...
describe("configProvider E2E - dark/light mode modifiers", () => {
  ...
  it("transforms scheme: modifier with config refs in both branches", () => {
    const extractCustomThemeSpy = vi.spyOn(configLoader, "extractCustomTheme").mockReturnValue({
      colors: {
        "primary-dark": "#111111",
...
describe("configProvider E2E - tw template literals", () => {
  it("transforms tw`` with config refs", () => {
    ...
    const output = transform(input, { configProvider: { importFrom: "./my-provider" } });
